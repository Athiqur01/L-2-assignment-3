...
The database is designed using an Entity Relationship Diagram (ERD) that clearly defines primary keys, foreign keys, and relationship cardinality. A one-to-many relationship exists between users and bookings, and a many-to-one relationship exists between bookings and vehicles, where each booking connects exactly one user to one vehicle. drawSQL has been used in this project for drawing ERD diagram.
